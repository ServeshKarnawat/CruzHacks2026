...
static uint32_t Serial_BRRCalculator(
    uint32_t fSysClk,
    uint32_t fPClk,
    uint32_t baudRate
);
...
static int8_t Nucleo_ConfigSysClk(void)
{
    ...
    // Initializes the RCC Oscillators according to the specified parameter in
...
/** Serial_BRRCalculator(fSysClk, fPClk, baudRate) -> uartDiv
 *
 * Convert system parameters into the proper format for use by the system clock
 * during set up.
 *
 * @param fSysClk   (uint32_t)  Main (system) clock frequency (Hz).
 * @param fPClk     (uint32_t)  APB clock frequency (Hz).
 * @param baudRate  (uint32_t)  Symbol rate desired.
 *
...
static int8_t Serial_Begin(void)
{
    ...
    huart2.Init.BaudRate = (uint32_t) Serial_BRRCalculator(
